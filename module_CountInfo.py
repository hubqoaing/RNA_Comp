...
import module_StatInfo as m_Stat

class CountInfo(object):
   def __init__(self,root_dir,l_samp,ltype,out_dir):
      self.l_sample = l_samp
      self.l_file   = [ "%s/%s/%s.%s.txt" % ( root_dir,samp,samp,ltype ) for samp in l_samp ]
   ...
   def load_mat( self,infile="",gen_col=1 ):
      self.gene = []
      self.cnt_matrix = []
   ...
   def cal_RPKM(self,M_gen_len,tophat_dir):
      ...
      for samp in self.l_sample:
         ...
         if os.path.isfile( align_log ):
            StatInfo    = m_Stat.TophatStat( align_log )
            StatInfo.read_infile()
         else:
            align_log   = "%s/%s/log" % (tophat_dir,samp)
            StatInfo    = m_Stat.HisatStat( align_log )
            StatInfo.read_infile()
      ...
      for i,gene in enumerate(self.gene):
         if (gene not in M_gen_len) or  ('max_len' not in M_gen_len[gene]) or M_gen_len[gene] == 0:
            print >>sys.stderr, "%s not in Gene_len list or with not valid maxlen" % (gene)
            continue
         self.RPKM_gene.append( gene )
         self.RPKM.append( (self.cnt_matrix[i]*10**9)/(M_gen_len[gene]['max_len']*np_sam_TotReads) )
