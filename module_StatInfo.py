...
class MappingStat(dict):
   def __init__( self ):
      self['statInfo']                         = {}
      self['statInfo']['totalRead']            = 0;
      self['statInfo']['mappedRead']           = 0;
      self['statInfo']['mappedPair']           = 0;
      self['statInfo']['mappingRate']          = "";
      self['statInfo']['mappingConcordantRate']= "";
...
class TophatStat(MappingStat):
   ...
   def __init__(self,infile):
      ...
      """
      super( TophatStat,self ).__init__()
      self['infile'] = infile
...
class HisatStat(MappingStat):
   
   def __init__(self,infile):
      super( HisatStat,self ).__init__()
      self['infile'] = infile
   ...
   def read_infile(self):
      ...
      self.__determine_item( ll_lines )
   ...
   def __determine_item(self,ll_lines):
      ...
      ### Single end
         ...
         self['statInfo']['mappedRead']            = int(ll_lines[3][0]) + int(ll_lines[4][0])
         self['statInfo']['mappedPair']            = int(ll_lines[3][0]) + int(ll_lines[4][0])
